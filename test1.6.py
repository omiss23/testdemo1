...
# Always copied onto the output row so an analyst sees the payment at a glance.
CONTEXT_COLUMNS = ["ORIGINATOR_NAME", "BENEFICIARY_NAME"]
...
def load_intelligence(path):
    ...
    if not Path(path).exists():
        sys.exit(f"ERROR: intelligence file not found: {path}")
...
def check_columns(df, keywords, transactions_file):
    ...
    required = {ID_COL, *COUNTRY_COLS, *CONTEXT_COLUMNS, *NAME_COLUMNS, *tagged, *searched}
...
def screen(transactions_file, mapping_file, intelligence_file, output_file):
    ...
    context_cols = [c for c in CONTEXT_COLUMNS if c != ID_COL]
    ...
    for spec in keywords:
        ...
        for field in spec["fields"]:
            ...
            for i in df.index[series.str.contains(pattern.pattern, regex=True) & in_scope]:
                ...
                row.update({c: df.at[i, c] for c in context_cols})
                row["DISPOSITION"] = disposition
                row["RULE_APPLIED"] = rule
                rows.append(row)

    columns = ([ID_COL, "RT_TYPE", "KEYWORD", "MATCH", "COLUMN", "TAG", "FIELD_VALUE",
                "SHAPE", "SCOPE_COUNTRY"] + context_cols + ["DISPOSITION", "RULE_APPLIED"])
    out = pd.DataFrame(rows, columns=columns)

    # a message survives if any one of its hits was not excluded
    if len(out):
        kept = out.groupby(ID_COL)["DISPOSITION"].transform(lambda s: (s != "EXCLUDED").any())
        out["FLAGGED"] = kept.map({True: "Y", False: "N"})
    else:
        out["FLAGGED"] = []
    ...
    if len(out):
        flagged = out[out["FLAGGED"] == "Y"][ID_COL].nunique()
