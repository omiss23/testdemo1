...
Transaction monitoring - keyword screening with per-keyword, per-column rules.
    ...
    python tm_screening.py            run the screening
    python tm_screening.py --sample   write demo mapping / intelligence / transaction files
...
# ----------------------------------------------------------------------------
# demo files - delete this once you are running on real data
# ----------------------------------------------------------------------------
def write_sample():
    p1 = pd.DataFrame([{"RT Type": "FT_IR_1", "Countries": "SY, IQ, LB, PK, AF"},
                       {"RT Type": "FT_IR_2", "Countries": "MA, DZ, TN, IN, TR"}])
    names = "ORIGINATOR_NAME, BENEFICIARY_NAME"
    names_ref = "ORIGINATOR_NAME, BENEFICIARY_NAME, REMITTANCE_INFO"
    p2 = pd.DataFrame([
        {"RT Type": "FT_IR_1", "Keyword": "JIHAD", "Message": names_ref,
         "Rules": "RETAIN_REF, EXCL_STRADDLE, EXCL_PP"},
        {"RT Type": "FT_IR_1", "Keyword": "HIJRAH", "Message": names_ref,
         "Rules": "RETAIN_REF, EXCL_PP"},
        {"RT Type": "FT_IR_1", "Keyword": "ISLAM", "Message": names,
         "Rules": "EXCL_STRADDLE, EXCL_PART_OF_WORD"},
        {"RT Type": "FT_IR_1", "Keyword": "ISLAMIC", "Message": names,
         "Rules": "EXCL_IN_BANK_PM, EXCL_IN_BANK_PP"},
        {"RT Type": "FT_IR_1", "Keyword": "AL HOL", "Message": names_ref,
         "Rules": "RETAIN_REF, EXCL_STRADDLE_PM, EXCL_STRADDLE_PP"},
        {"RT Type": "FT_IR_1", "Keyword": "AL ROJ", "Message": names,
         "Rules": "EXCL_STRADDLE_PM"},
        {"RT Type": "FT_IR_1", "Keyword": "KURD", "Message": names,
         "Rules": "EXCL_SURNAME_PM, EXCL_SURNAME_PP"},
        {"RT Type": "FT_IR_2", "Keyword": "CHARITY", "Message": names_ref,
         "Rules": "RETAIN_REF, EXCL_IN_SECULAR, EXCL_IN_RELIGIOUS"},
        {"RT Type": "FT_IR_2", "Keyword": "AFGHAN", "Message": "ORIGINATOR_NAME, BENEFICIARY_NAME, PAYMENT_DETAILS",
         "Rules": "EXCL_PART_OF_WORD, EXCL_ON_TOPONYM"},
        {"RT Type": "FT_IR_2", "Keyword": "GAZA", "Message": names,
         "Rules": "EXCL_PART_OF_WORD, REVIEW_ON_TOPONYM"},
        {"RT Type": "FT_IR_2", "Keyword": "HARYANA", "Message": names,
         "Rules": "EXCL_ON_TOPONYM"},
    ])

    intel = {
        "TOPONYM": ["ISLAMABAD", "ISLAMKOT", "AFGHANISTAN", "CASABLANCA", "HARYANA", "KURDISTAN",
                    "GAZIANTEP", "GAZA", "ALEPPO", "RAQQA", "IDLIB", "KABUL", "KANDAHAR",
                    "JALALABAD", "PESHAWAR", "KARACHI"],
        "SECULAR": ["EVENT", "GALA", "BALL", "DINNER", "CONCERT", "FESTIVAL", "MARATHON", "RUN",
                    "RACE", "RAFFLE", "AUCTION", "TOURNAMENT", "GOLF", "SHOP", "STORE", "SPORTS", "CLUB"],
        "RELIGIOUS": ["SISTERS OF", "BROTHERS OF", "DAUGHTERS OF", "CONGREGATION", "PARISH", "DIOCESE",
                      "CATHEDRAL", "CHURCH", "CHAPEL", "MONASTERY", "CONVENT", "MISSION", "SAINT",
                      "CATHOLIC", "PROTESTANT", "METHODIST", "ANGLICAN", "BAPTIST", "SALVATION ARMY"],
        "BANK": ["BANK", "BANKING", "BANQUE", "BANCO", "BANCA", "BK", "FINANCIAL", "FINANCE",
                 "CREDIT", "SAVINGS", "COOPERATIVE"],
    }

    rows = [
        ("TXN001", "Cristal Holding", "SY", "Gulf Trade FZE", "AE", "INVOICE 4471", ""),
        ("TXN002", "Cristal Rojana Trading", "IQ", "Delta Textiles Ltd", "GB", "PO 88", ""),
        ("TXN003", "Rami Haddad", "LB", "Jihad El-Khoury", "FR", "FAMILY SUPPORT", ""),
        ("TXN004", "Jihad Traders LLC", "SY", "Global Foods Inc", "GB", "GROCERY", ""),
        ("TXN005", "Yusuf Demir", "SY", "Rami Haddad", "LB", "DONATION IN SUPPORT OF JIHAD BRIGADE", ""),
        ("TXN006", "Delta Textiles Ltd", "GB", "Islamabad Traders", "PK", "COTTON SHIPMENT", ""),
        ("TXN007", "Ahmed Khan", "PK", "Islamic Bank of Development", "SA", "LOAN REPAYMENT", ""),
        ("TXN008", "Atlas Cement SARL", "MA", "Monaco Charity Event", "MC", "SPONSORSHIP", ""),
        ("TXN009", "Sisters of Charity of Australia", "AU", "Vidya Trust", "IN", "GRANT", ""),
        ("TXN010", "Maroc Logistics SARL", "MA", "Iberia Freight SA", "ES", "FREIGHT",
         "DELIVERY TO BOULEVARD D'AFGHANISTAN, CASABLANCA"),
        ("TXN011", "Farid Afghanatan", "IN", "Pearl Jewels Pvt Ltd", "AE", "PURCHASE", ""),
        ("TXN012", "Sofia Rossi", "IT", "Florian Gazan", "TR", "RENT", ""),
        ("TXN013", "Al Hol Relief Fund", "SY", "Kamal Nasser", "LB", "OPERATIONAL COSTS", ""),
        ("TXN014", "Anadolu Metal AS", "IQ", "Mehmet Kurdoglu", "TR", "SCRAP METAL", ""),
        ("TXN015", "Jihad Traders LLC", "US", "Global Foods Inc", "GB", "GROCERY", ""),
        ("TXN016", "Al Noor Exchange", "AE", "Hijrah Ben Salman", "PK", "REMITTANCE", ""),
        ("TXN017", "Nour Textiles", "SY", "Fatima Zahra", "FR", "PAYMENT FOR HIJRAH TRAVEL", ""),
        ("TXN018", "Anand Exports", "IN", "Gaza Aid Committee", "PS", "RELIEF FUNDING", ""),
        ("TXN019", "Surat Textiles Pvt Ltd", "IN", "Haryana India Logistics", "IN", "FREIGHT", ""),
        ("TXN020", "Al Hol Trading, Karachi", "PK", "Delta Textiles Ltd", "GB", "TEXTILE ORDER", ""),
    ]
    tx = pd.DataFrame([{
        "TXN_ID": r[0], "VALUE_DATE": f"2026-07-{i + 1:02d}", "AMOUNT": f"{(i + 3) * 4150:.2f}", "CURRENCY": "EUR",
        "ORIGINATOR_NAME": r[1], "ORIGINATOR_COUNTRY": r[2], "ORIGINATOR_FI_COUNTRY_CD": r[2],
        "BENEFICIARY_NAME": r[3], "BENEFICIARY_COUNTRY": r[4], "BENEFICIARY_FI_COUNTRY_CD": r[4],
        "REMITTANCE_INFO": r[5], "PAYMENT_DETAILS": r[6],
    } for i, r in enumerate(rows)])

    with pd.ExcelWriter(MAPPING_FILE, engine="openpyxl") as writer:
        p1.to_excel(writer, sheet_name=SHEET_P1, index=False)
        p2.to_excel(writer, sheet_name=SHEET_P2, index=False)
    with pd.ExcelWriter(INTELLIGENCE_FILE, engine="openpyxl") as writer:
        for name, values in intel.items():
            pd.DataFrame({"Value": values,
                          "Status": ["ACTIVE"] * len(values),
                          "Source": [""] * len(values)}).to_excel(writer, sheet_name=name, index=False)
    tx.to_csv(TRANSACTIONS_FILE, index=False)
    print(f"wrote {MAPPING_FILE}, {INTELLIGENCE_FILE} and {TRANSACTIONS_FILE}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("--sample", action="store_true", help="write demo files and exit")
